defmodule SootSegments.Resource.SegmentRow.Transformers.Inject do
  @moduledoc false
  use Spark.Dsl.Transformer

  alias Ash.Resource.Builder
  alias SootSegments.Resource.SegmentRow.Preparations
  ...
  defp add_attributes(dsl_state) do
    with {:ok, dsl_state} <- ensure_uuid_primary_key(dsl_state),
         ...
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :source_stream, :atom,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :granularity, :atom,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :current_version_id, :uuid,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
         ...
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :target, :string, public?: true),
    ...
    with {:ok, dsl_state} <-
         ...
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :create, :create,
             primary?: true,
             accept: [:name, :source_stream, :granularity, :current_version_id, :target]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :update, :update,
             primary?: true,
             require_atomic?: false,
             accept: [:current_version_id, :target]
           ),
         {:ok, dsl_state} <-
  ...
  defp add_code_interface(dsl_state) do
    with {:ok, dsl_state} <-
           Builder.add_new_interface(dsl_state, :create,
             args: [:name, :source_stream, :granularity, :current_version_id]
           ),
