defmodule SootSegments.Resource.SegmentVersion.Transformers.Inject do
  @moduledoc false
  use Spark.Dsl.Transformer

  alias Ash.Resource.Builder
  alias SootSegments.Resource.SegmentVersion.Preparations
  alias Spark.Dsl.Transformer

  @statuses [:current, :deprecated, :retired]
  ...
  defp add_attributes(dsl_state) do
    with {:ok, dsl_state} <- ensure_uuid_primary_key(dsl_state),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :segment_name, :atom,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
         ...
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :definition, :map,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :date_floor, :utc_datetime_usec,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
         ...
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :materialized_target, :string, public?: true),
  ...
  defp add_identities(dsl_state, domain) do
    with {:ok, dsl_state} <-
           Builder.add_new_identity(
             dsl_state,
             :unique_fingerprint_per_segment,
             [:segment_name, :fingerprint],
             pre_check_with: domain
           ) do
      Builder.add_new_identity(
        dsl_state,
        :unique_version_per_segment,
  ...
  defp add_actions(dsl_state) do
    with {:ok, dsl_state} <-
         ...
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :update, :promote,
             accept: [],
             require_atomic?: false,
             changes: [
               Builder.build_action_change(
                 {Ash.Resource.Change.SetAttribute, attribute: :status, value: :current}
               )
             ]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :read, :for_segment,
             arguments: [
               Builder.build_action_argument(:segment_name, :atom, allow_nil?: false)
             ],
             preparations: [Builder.build_preparation(Preparations.ForSegment)]
           ) do
  ...
  defp add_code_interface(dsl_state) do
    with {:ok, dsl_state} <-
         ...
         {:ok, dsl_state} <- Builder.add_new_interface(dsl_state, :promote),
         {:ok, dsl_state} <-
           Builder.add_new_interface(dsl_state, :for_segment, args: [:segment_name]) do
      Builder.add_new_interface(dsl_state, :get_by_fingerprint,
        args: [:segment_name, :fingerprint]
      )
    end
  end
end
